#!/usr/bin/env python3
"""
Extract FastESM2-650M embeddings for 150bp sliding window reads (10% subset).

Runs complete extraction pipeline on 10% sample to estimate runtime:
1. Extract DNABERT-S embeddings (768-dim)
2. Extract FastESM2-650M protein embeddings (1280-dim)
3. Merge to 2048-dim features
4. Save as .pt files

Usage:
    python scripts/extract_150bp_embeddings_subset.py
"""
import sys
...
def patch_dnabert_s_triton():
    ...
    for file_path in files:
        ...
        if 'tl.trans(k)' in content:
            logger.info(f"Already patched: {file_path}")
            patched_count += 1
            continue
    ...
    logger.info(f"Patched {patched_count} file(s)")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("150bp Read Embedding Extraction (10% Subset Test)")
    logger.info("=" * 60)
    ...
    # Apply DNABERT-S patch
    patch_dnabert_s_triton()

    # Input/output paths (10% subset)
    viral_fasta = "test_150bp_viral_10pct.fasta"
    nonviral_fasta = "test_150bp_nonviral_10pct.fasta"

    output_dir = Path("data/test_150bp_subset")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("Processing VIRAL 150bp reads (10% subset)")
    logger.info("=" * 60)
    ...
    extract_fast_esm(
        ...
        toks_per_batch=20480  # 10x larger batch for 150bp reads
    )
    ...
    # Validate non-viral merged data
    ...
    logger.info(f"Expected: ({len(nonviral_data['ids'])}, {MERGED_DIM})")
    assert nonviral_data['data'].shape[1] == MERGED_DIM, "Dimension mismatch!"

    total_elapsed = time.time() - start_time

    # Summary with runtime estimates
    logger.info("\n" + "=" * 60)
    logger.info("EXTRACTION COMPLETE (10% SUBSET)")
    logger.info("=" * 60)
    logger.info(f"Viral 150bp windows: {len(viral_data['ids'])} (10% of 366,587)")
    logger.info(f"Non-viral 150bp windows: {len(nonviral_data['ids'])} (10% of 334,166)")
    logger.info(f"Total windows: {len(viral_data['ids']) + len(nonviral_data['ids'])} (10% of ~700K)")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("RUNTIME ANALYSIS")
    logger.info("=" * 60)
    logger.info(f"Viral subset (10%): {viral_elapsed/60:.1f} minutes")
    logger.info(f"Non-viral subset (10%): {nonviral_elapsed/60:.1f} minutes")
    logger.info(f"Total (10%): {total_elapsed/60:.1f} minutes")
    logger.info(f"\nEstimated full runtime (100%):")
    logger.info(f"  ~{total_elapsed*10/3600:.1f} hours ({total_elapsed*10/60:.0f} minutes)")

#!/usr/bin/env python3
"""
Extract FastESM2-650M embeddings for 150bp sliding window reads.
...
Usage:
    python scripts/extract_150bp_embeddings.py
"""
import sys
...
def main():
    logger.info("=" * 60)
    logger.info("150bp Read Embedding Extraction (FastESM2-650M)")
    logger.info("=" * 60)
    ...
    # Input/output paths
    viral_fasta = "test_150bp_viral.fasta"
    nonviral_fasta = "test_150bp_nonviral.fasta"

    output_dir = Path("data/test_150bp")
    ...
    logger.info("=" * 60)
    logger.info(f"Viral 150bp windows: {len(viral_data['ids'])}")
    logger.info(f"Non-viral 150bp windows: {len(nonviral_data['ids'])}")
    logger.info(f"Total windows: {len(viral_data['ids']) + len(nonviral_data['ids'])}")

#!/usr/bin/env python3
"""
Extract FastESM2-650M embeddings for 50K random 150bp reads.

Fast benchmark approach (~20-30 mins) for 150bp short read performance.

Usage:
    python scripts/extract_150bp_embeddings_50k.py
"""
import sys
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("150bp Read Embedding Extraction (50K Sample)")
    logger.info("=" * 60)
    ...
    protein_model = protein_model.to(device)  # Move to GPU!
    protein_model.eval()

    # Process viral sequences
    logger.info("\n" + "=" * 60)
    logger.info("Processing VIRAL 150bp reads (50K sample)")
    logger.info("=" * 60)
    ...
    logger.info("\n" + "=" * 60)
    logger.info("Processing NON-VIRAL 150bp reads (50K sample)")
    logger.info("=" * 60)
    ...
    merge_data(
        ...
        data_type="host"  # Use 'host' for non-viral (label=0)
    )
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.info("EXTRACTION COMPLETE (50K Sample)")
    logger.info("=" * 60)
    logger.info(f"Viral 150bp windows: {len(viral_data['ids'])} (52.4%)")
    logger.info(f"Non-viral 150bp windows: {len(nonviral_data['ids'])} (47.6%)")

...
must_haves:
  ...
  key_links:
    - from: "virnucpro/pipeline/gpu_worker.py"
      ...
      via: "get_env_config() for FP16 disable check"
  ...
  <action>
    ...
    **Functions to extract:**
    ...
    5. `_load_model(model_config: Dict, device: torch.device, logger) -> Tuple[torch.nn.Module, Any]`
       ...
       **Important**: Replace the inline `should_use_fp16()` call with EnvConfig:
       ```python
       ...
       ```
       This replaces the current pattern of calling `should_use_fp16()` from `virnucpro.utils.precision`.
...
- FP16 env var check uses EnvConfig instead of should_use_fp16()

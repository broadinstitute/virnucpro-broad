...
must_haves:
  truths:
    ...
    - "Tests verify work stealing produces identical outputs"
  artifacts:
...
<objective>
Create comprehensive test suite for work stealing, GPU weighting, and load distribution fairness including output validation.

Purpose: Ensure load balancing features work correctly and improve GPU utilization without changing results.
...
<task type="auto">
  <name>Task 3: Create load balancing integration tests with output validation</name>
  ...
  <action>
    ...
    TEST-06 requirement - Add critical test:
    11. test_work_stealing_identical_outputs() - End-to-end test comparing outputs:
        - Run pipeline on test data WITHOUT work stealing (baseline)
        - Run pipeline on same data WITH work stealing enabled
        - Compare prediction_results.txt files byte-for-byte
        - Verify CSV outputs are identical (same predictions, same order)
        - Test with various file distributions to ensure deterministic results
        - Use small test dataset for fast execution
    ...
    Assert that outputs remain identical regardless of work distribution.
  </action>
  <verify>grep -n "test_idle_gpu_steals_work" tests/test_load_balancing.py && grep -n "test_work_stealing_identical_outputs" tests/test_load_balancing.py</verify>
  <done>Integration tests validate complete load balancing system and identical outputs</done>
</task>
...
- TEST-06: End-to-end test verifies work stealing produces identical outputs
...
- Work stealing mode produces identical predictions to baseline mode

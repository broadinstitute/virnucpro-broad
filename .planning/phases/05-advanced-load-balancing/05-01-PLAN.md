...
<task type="auto">
  ...
  <action>
    ...
    - Imbalance threshold: max_queue >= 3 AND (max > 2×min OR max - min >= 3) - conservative to avoid thrashing
...
<task type="auto">
  ...
  <action>
    ...
    - Pass shared queues to workers via Pool initializer pattern (not as arguments - spawn context issue)
    ...
    - Pass queues via Pool initializer: Set global _work_queues variable in initializer function
    - Workers access shared queues via: global _work_queues[gpu_id]
    - Use same pattern as existing _esm_model, _batch_converter globals
...
- Workers access shared queues via global _work_queues[gpu_id] pattern
</verification>

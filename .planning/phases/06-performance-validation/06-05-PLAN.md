...
must_haves:
  ...
  key_links:
    ...
    - from: "virnucpro/cli/main.py"
      to: "virnucpro.cli.benchmark"
      via: "benchmark command registration"
      pattern: "from virnucpro\\.cli import benchmark"
---
...
<task type="auto">
  ...
  <files>virnucpro/cli/benchmark.py, virnucpro/cli/main.py</files>
  <action>
    ...
    2. Add command-line arguments in benchmark.py:
    ...
    5. Register command in virnucpro/cli/main.py:
       - Import benchmark module: from virnucpro.cli import benchmark
       - Add benchmark subcommand to argument parser:
         * In add_subparsers() section, add benchmark_parser = subparsers.add_parser('benchmark', help='Run performance benchmarks')
         * Call benchmark.add_arguments(benchmark_parser) to add benchmark-specific args
         * In main() function, add elif args.command == 'benchmark': benchmark.main(args)
       - Follow existing pattern used for profile, predict, and other subcommands

    6. Add example usage to help text:
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    3. Verify markdown report shows:
       - GPU scaling results with speedup >= 1.6x for 2 GPUs
       - Performance metrics showing <10 hour projection for 4 GPUs
       - GPU utilization >= 80% for DNABERT and ESM-2 stages
       - No performance regressions flagged (all tests passed)
    4. Confirm JSON report contains:
       - gpu_scaling data with speedup ratios
       - stage_throughput with GPU utilization percentages
       - memory_usage metrics
       - equivalence validation results (rtol <= 1e-3)
  </how-to-verify>
  <resume-signal>Type "approved" if all performance criteria met (scaling >= 1.6x, GPU util >= 80%, <10hr projection), or describe specific failures</resume-signal>
</task>

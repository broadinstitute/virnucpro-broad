...
must_haves:
  ...
  key_links:
    ...
    - from: "tests/benchmarks/test_end_to_end.py"
      ...
      via: "NvitopMonitor import for tracking"
      pattern: "from virnucpro\\.utils\\.gpu_monitor import NvitopMonitor"
    - from: "tests/benchmarks/test_memory_usage.py"
      to: "virnucpro.utils.gpu_monitor"
      via: "NvitopMonitor import for memory tracking"
      pattern: "from virnucpro\\.utils\\.gpu_monitor import NvitopMonitor"
---
...
<task type="auto">
  ...
  <action>
    ...
    2. Add test_typical_sample_under_10_hours():
       ...
       - Instantiate NvitopMonitor and start monitoring before pipeline execution
       - Run full pipeline with 4 GPUs and all optimizations
       - Stop monitoring and collect metrics after completion
       - Measure total time and project to full sample
       - Assert projected time < 10 hours (with 10% tolerance)
       - Generate detailed timing report with GPU utilization data
...
<task type="auto">
  ...
  <action>
    ...
    1. Create TestMemoryUsage class:
       - Import NvitopMonitor from virnucpro.utils.gpu_monitor for memory tracking
    ...
    2. Add test_memory_stability():
       ...
       - Instantiate NvitopMonitor and start continuous monitoring
       - Track GPU memory usage every second via NvitopMonitor
    ...
    3. Add test_peak_memory_per_stage():
       - Start NvitopMonitor before each stage
       - Measure peak memory for each pipeline stage
       - Identify memory-intensive operations
       - Verify peaks stay within GPU capacity
       - Generate memory usage timeline chart data from NvitopMonitor logs

    4. Add test_memory_with_large_batches():
       - Test with maximum batch sizes
       - Use NvitopMonitor to track memory during batch processing
    ...
    5. Add test_persistent_models_memory():
       ...
       - Use NvitopMonitor for continuous memory tracking
    ...
    6. Create memory report:
       - Peak memory per GPU (from NvitopMonitor.get_peak_memory_usage())
    ...
    Use torch.cuda.memory_stats() and NvitopMonitor for comprehensive tracking.
...
- GPU monitoring integrated throughout all tests
...
- Each optimization's impact quantified with GPU metrics
- Detailed performance reports generated with monitoring data
</success_criteria>

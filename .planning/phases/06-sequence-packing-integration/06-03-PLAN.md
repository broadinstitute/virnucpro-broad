...
must_haves:
  truths:
    ...
    - "Dtype validated for FlashAttention (assert embeddings.dtype in [FP16, BF16])"
...
3. **CRITICAL: Validate dtype for FlashAttention compatibility**
   ```python
   # After embedding layer
   embeddings = self.model.embed_tokens(input_ids)

   # Verify dtype - FlashAttention requires FP16/BF16
   ...
   ```
4. For each layer, replace attention with flash_attn_varlen_func:
...
6. Return packed representations

...
must_haves:
  truths:
    ...
    - "Warning logged when packing efficiency < 80%"
  artifacts:
...
Also add efficiency logging to GreedyPacker.pack_sequences:
```python
efficiency = self.compute_efficiency(batches)
if efficiency < 0.8:
    logger.warning(f"Low packing efficiency: {efficiency:.1%}")
else:

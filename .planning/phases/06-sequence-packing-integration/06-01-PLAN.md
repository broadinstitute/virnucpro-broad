...
must_haves:
  truths:
    ...
    - "pack_sequences() designed for buffer-based usage (1000-5000 sequences, not DataLoader micro-batches)"
    - "Packing efficiency: 92-94% with 2000-sequence buffer, >90% with 1000+ buffer"
    - "Sequences exceeding max_length are truncated with warning (N-terminal preservation)"
...
Purpose: Pack variable-length sequences into token-budget batches to maximize GPU utilization. FFD achieves ~92-94% packing efficiency with buffer-based usage (ARCH-11). pack_sequences() is designed to operate on buffers of 1000-5000 sequences accumulated by VarlenCollator, NOT individual DataLoader batches. Dynamic token budget (PACK-03) adapts to available GPU memory.
...
1. `pack_sequences(sequences: List[Dict]) -> List[List[Dict]]`:
   - **BUFFER-BASED DESIGN:** Optimized for 1000-5000 sequence buffers (not 8-32 DataLoader micro-batches)
   ...
   - Truncate sequences >max_sequence_length with warning and `truncated=True` flag (N-terminal only; Phase 10 will add REJECT/C-TERMINAL/SLIDING_WINDOW options)
   - Return list of batches, each a list of sequence dicts
   - **Efficiency:** 92-94% with 2000-sequence buffer, ~90% with 1000, ~70% with <500

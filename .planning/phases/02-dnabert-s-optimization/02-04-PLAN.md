...
<task type="auto">
  ...
  <action>
    ...
    4. Test performance improvements:
       - test_dnabert_throughput_improvement() - Measure speedup (expect 3-4x)
  ...
  <verify>python -m pytest tests/test_integration_dnabert_multi_gpu.py -v --tb=short -k "test_dnabert"</verify>
  <done>Integration tests created covering DNABERT-S multi-GPU processing, correctness, and performance</done>
</task>
...
<task type="auto">
  ...
  <verify>grep -n "DNABERT-S Optimization" docs/optimization_guide.md && grep -n "--dnabert-batch-size" docs/optimization_guide.md</verify>
  <done>Optimization guide created with clear explanations of DNABERT-S and ESM-2 GPU features</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>DNABERT-S multi-GPU optimization with parallel processing, BF16 support, and profiling tools</what-built>
  <how-to-verify>
    ...
    2. Test the actual pipeline with DNABERT-S optimization:
       ```bash
    ...
    4. Test batch size tuning:
       ```bash
       virnucpro predict ... --dnabert-batch-size 4096
       ```

    5. Review the optimization guide:
       ```bash
    ...
    - 3-4x throughput improvement with 4 GPUs
...
- Performance tests show 3-4x throughput improvement

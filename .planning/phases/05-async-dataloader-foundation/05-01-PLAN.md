...
1. **Constructor (`__init__`):**
   ...
   - Accept `max_tokens_per_batch: int = 4096` (token budget per batch)
   - Store both as instance attributes
   - Get padding_idx from alphabet: `self.padding_idx = batch_converter[0].padding_idx` (typically 1)
...
2. **Call method (`__call__`):**
   ...
   **CRITICAL - Handle ESM padding correctly:**
   - ESM batch_converter returns PADDED 2D tensor (batch_size × max_seq_len)
   - Must strip padding tokens before concatenating
   - For each sequence row in tokens:
     - Find actual length by locating first padding token: `seq_len = (tokens[i] != self.padding_idx).sum().item()`
     - Extract unpadded tokens: `seq_tokens = tokens[i, :seq_len].tolist()`

   **Build packed format:**
   - Initialize `all_tokens = []`, `cu_seqlens = [0]`, `sequence_ids = []`
   - For each sequence:
     - Get unpadded tokens as described above
     - **Edge case check:** If this is the FIRST sequence and `len(seq_tokens) > max_tokens_per_batch`:
       - Still include it (partial batch with 1 sequence)
       - Log warning: "Sequence {seq_id} exceeds max_tokens_per_batch ({len} > {max})"
     - **Normal case:** If `cu_seqlens[-1] + len(seq_tokens) > max_tokens_per_batch`:
       - Stop packing (don't include this sequence)
       - Break loop
     - Extend all_tokens with seq_tokens: `all_tokens.extend(seq_tokens)`
     - Append CUMULATIVE length: `cu_seqlens.append(len(all_tokens))`
       - ⚠️ cu_seqlens format: [0, len1, len1+len2, len1+len2+len3, ...]
       - Example: 3 sequences of lengths [5, 7, 3] → cu_seqlens = [0, 5, 12, 15]
     - Track sequence ID: `sequence_ids.append(labels[i])`

   - Return dict:
     - `'input_ids'`: `torch.tensor(all_tokens, dtype=torch.long)` (1D concatenated)
     - `'cu_seqlens'`: `torch.tensor(cu_seqlens, dtype=torch.int32)` (cumulative boundaries)
     - `'max_seqlen'`: max individual sequence length in packed batch
     - `'sequence_ids'`: list of sequence IDs that were packed
     - `'num_sequences'`: `len(cu_seqlens) - 1` (cu_seqlens has N+1 elements for N sequences)

3. **Add module docstring** explaining:
   - Purpose: Tokenize in main process for CUDA safety
   - Output format: Packed 1D tensor with cu_seqlens for FlashAttention varlen
   - cu_seqlens format: Cumulative sequence boundaries [0, len1, len1+len2, ...]
   - Padding handling: Strips ESM padding_idx tokens before packing
   - Integration: Used as collate_fn with SequenceDataset

Import: torch, logging, List, Dict, Any
  </action>
  <verify>
```bash
python -c "
...
# Create collator with small max for testing
collator = VarlenCollator(batch_converter, max_tokens_per_batch=100)
...
# Verify cu_seqlens format (cumulative)
assert result['cu_seqlens'][0] == 0, 'cu_seqlens must start at 0'
assert result['cu_seqlens'][-1] == len(result['input_ids']), 'cu_seqlens end must equal total tokens'

# Verify no padding tokens in packed output
padding_idx = 1  # ESM default
assert padding_idx not in result['input_ids'], 'Found padding tokens in packed output'

print(f'VarlenCollator test passed: {result[\"num_sequences\"]} sequences, {len(result[\"input_ids\"])} tokens')
print(f'cu_seqlens: {result[\"cu_seqlens\"].tolist()}')
"
```
  </verify>
  <done>VarlenCollator tokenizes sequences, strips ESM padding, and produces packed format with cumulative cu_seqlens for FlashAttention varlen</done>
</task>

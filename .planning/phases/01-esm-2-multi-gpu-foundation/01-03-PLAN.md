...
<task type="auto">
  ...
  <action>
    ...
    **tests/test_parallel_esm.py:**
    ...
    2. TestAssignFilesRoundRobin class:
       - test_empty_files(): Test with empty file list
       - test_single_worker(): All files go to one worker
       - test_balanced_assignment(): Files distributed evenly (round-robin pattern)
       - test_uneven_assignment(): Test with files not evenly divisible by workers
    ...
    **tests/test_work_queue.py:**
    ...
    3. TestBatchQueueManager class:
       ...
       - test_worker_function_validation(): Test signature validation

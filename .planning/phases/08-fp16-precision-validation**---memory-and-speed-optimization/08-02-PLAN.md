...
must_haves:
  truths:
    ...
    - "gpu_worker passes enable_fp16 from model_config to ESM-2 model loader"
    - "gpu_worker passes enable_fp16 from model_config to DNABERT-S model loader"
...
<objective>
Add NaN/Inf numerical stability detection to the inference pipeline and wire FP16 flag through gpu_worker for both ESM-2 and DNABERT-S.

Purpose: FP16 can cause overflow in LayerNorm or attention. NaN/Inf detection catches this early with actionable error messages. The gpu_worker must pass the FP16 flag to model loading for both model types in multi-GPU inference.

Output: NaN/Inf detection in async_inference.py, FP16 passthrough in gpu_worker.py (both ESM-2 and DNABERT-S), unit tests for both.
</objective>
...
<task type="auto">
  ...
  **ESM-2 loading (around line 116-124):** Pass enable_fp16:
  ```python
  if model_type == 'esm2':
  ...
  **DNABERT-S loading (add elif branch after ESM-2):** Add new model type support with enable_fp16:
  ```python
  ...
  Update the model_config docstring to include `'model_type': 'esm2' or 'dnabert'`.
  </action>
  <verify>
  ...
  <done>
  NaN/Inf detection added after every forward pass in _run_inference (both packed and unpacked paths). Error messages include VIRNUCPRO_DISABLE_FP16 hint. gpu_worker passes enable_fp16 from model_config to both load_esm2_model and load_dnabert_model.
  </done>
</task>
...
- `grep -n 'enable_fp16' virnucpro/pipeline/gpu_worker.py` -- wired through for both esm2 and dnabert
</verification>
...
- gpu_worker passes enable_fp16 from model_config to both ESM-2 and DNABERT-S model loaders

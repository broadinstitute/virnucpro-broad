...
must_haves:
  truths:
    ...
    - "FP32 baseline throughput established within same benchmark run for fair comparison"
...
<objective>
Benchmark FP16 throughput against the FP32 baseline (Phase 7 default) to quantify runtime reduction.

Purpose: Phase 7 runs in FP32 (the current production baseline). Phase 8 switches to FP16. This benchmark establishes the FP32 baseline and FP16 performance within the same controlled run, then computes the improvement ratio. Per CONTEXT.md, total runtime reduction is the primary metric, with 1.8-2x throughput target. This is the one-time Phase 8 validation benchmark, not the on-demand diagnostic (which uses --fp32-compare flag at runtime).

Output: Benchmark test that loads the same model in FP32 (Phase 7 baseline) and FP16 (Phase 8), runs identical workloads, and prints a comparison table showing runtime reduction and throughput improvement.
</objective>
...
<task type="auto">
  ...
  **Module header:**
  ```python
  """FP16 vs FP32 (Phase 7 baseline) throughput benchmarking for ESM-2.

  Establishes FP32 baseline (Phase 7 default precision) and FP16 (Phase 8)
  within the same controlled benchmark run. Measures total runtime reduction
  (primary metric), tokens/second, and peak GPU memory.
  ...
  **Helper function for running benchmark:**
  ...
  def benchmark_model(model, batch_converter, sequences, device, num_warmup=5, num_batches=50):
      ...
      Measures total runtime (primary metric per CONTEXT.md) and derived
      throughput metrics (tokens/sec, sequences/sec) to isolate GPU
      improvement from I/O.
      """
  ...
  **class TestFP16Throughput:**
  ...
  - `test_fp16_vs_fp32_throughput` - The main benchmark test:
    ...
    2. Load model in FP32 (Phase 7 baseline precision), benchmark it
    3. Delete FP32 model, clear GPU cache
    4. Load model in FP16 (Phase 8 precision), benchmark it
    5. Calculate speedup ratio: `fp16_tokens_per_sec / fp32_tokens_per_sec`
    6. Calculate runtime reduction: `1 - (fp16_elapsed / fp32_elapsed)`
    7. Calculate memory savings: `1 - (fp16_memory / fp32_memory)`
    8. Print comprehensive summary table:
       ```
       Phase 8 FP16 vs Phase 7 FP32 Baseline
       =======================================
       ...
       ---------------------------------------------------------
       Target: 1.8-2.0x throughput improvement
       ```
    9. Assert throughput improvement >1.0x (FP16 should be at least as fast)
    10. Assert peak memory FP16 <= FP32 (should not use more memory)

  - `test_fp16_memory_reduction` - Load model in FP16, check peak memory.
    1. Load model in FP32 (Phase 7 baseline), record `torch.cuda.max_memory_allocated`
    2. Delete model, clear cache
    3. Load model in FP16, record peak memory
    4. Print memory comparison: "FP32 (Phase 7 baseline): X.XGB, FP16 (Phase 8): Y.YGB"
  ...
  The printed table gives the user clear data to evaluate against the 1.8-2x target.
  </action>
  <verify>
  ...
  <done>
  FP16 vs FP32 throughput benchmark created. Establishes FP32 baseline (Phase 7 precision) and FP16 (Phase 8) in same run. Reports total runtime reduction (primary metric), tokens/sec, and memory savings. Results printed as comparison table referencing Phase 7 baseline.
  </done>
</task>
...
- Total runtime comparison printed (Phase 7 FP32 vs Phase 8 FP16)
...
<success_criteria>
- Benchmark establishes FP32 baseline (Phase 7 precision) within the same run
